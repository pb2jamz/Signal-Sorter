...
export const useItems = () => {
  ...
  const loadItems = useCallback(async (silent = false) => {
    ...
    if (!silent) {
      console.log('[Items] Loading for user:', user.id)
      setLoading(true)
    }
    ...
    try {
      ...
      console.log('[Items] Fetching from Supabase...')
      ...
      console.log('[Items] Supabase response:', { data: data?.length, error: fetchError })

      if (fetchError) {
        console.error('[Items] Fetch error:', fetchError)
        throw fetchError
      }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`items-${user.id}-${Date.now()}`)
      .on(
        'postgres_changes',
        {
          event: '*',
